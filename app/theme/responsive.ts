...
export const squareImageSize = (designSize: number) => {
  const scaleFactor = Math.min(widthPercent, heightPercent)
  const responsiveSize = designSize * scaleFactor
  return Number(responsiveSize.toFixed(1))
}
